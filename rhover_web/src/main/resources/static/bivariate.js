...
function renderBivariatePlot(fieldInstanceId1, fieldInstanceId2, divId, width, height) {
	
	const chartWidth = width - margin.left - margin.right;
	const chartHeight = height - margin.top - margin.bottom;
	
	const url = "/rest/data/bivariate?field_instance_id_1=" + fieldInstanceId1
		+ "&field_instance_id_2=" + fieldInstanceId2;
	
	d3.csv(url, function(error, data) {
		console.log(data);
		
		// Extract X and Y axis labels
		const xLabel = data.columns[0];
		const yLabel = data.columns[1];
...
$(function() {
	renderBivariatePlot(fieldInstanceId1, fieldInstanceId2, "chart", 600, 600);
});